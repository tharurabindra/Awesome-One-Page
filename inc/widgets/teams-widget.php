...
/**
 * Teams Widget section.
 */
class aop_team_widget extends WP_Widget {
  function __construct() {
      $widget_ops = array( 'classname' => 'widget_team_block', 'description' => esc_html__( 'Display some pages as team.', 'awesome-one-page' ) );
      $control_ops = array( 'width' => 200, 'height' =>250 );
        parent::__construct( false, $name = esc_html__( 'AOP: Teams', 'awesome-one-page' ), $widget_ops, $control_ops);
    }
    ...
    function form( $instance ) {
      ...
      <div class="aop-team">
        <div class="aop-admin-input-wrap">
          <p><?php esc_html_e('This widget displays all pages related to Single Team Template.', 'awesome-one-page'); ?></p>
          <p><em><?php esc_html_e('Tip: to rearrange the team order, edit each team page and add a value in Page Attributes > Order', 'awesome-one-page'); ?></em></p>
          ...
          <ul class="aop-widget-repetable-field-items">
            <li class="aop-widget-accordion-item">
              ...
              <div class="accordion-content">
                ...
                <div class="aop-admin-input-wrap">
                  <p><em><?php esc_html_e('Tip: Enter the Team Section ID and use same for Menu item. Only used for One Page Menu.', 'awesome-one-page'); ?></em></p>
                </div><!-- .aop-admin-input-wrap -->

                <div class="aop-admin-input-wrap">
                  <label for="<?php echo $this->get_field_id( 'section_id' ); ?>"><?php esc_html_e( 'Section ID', 'awesome-one-page' ); ?></label>
                  <input type="text" id="<?php echo $this->get_field_id( 'section_id' ); ?>" name="<?php echo $this->get_field_name( 'section_id' ); ?>" value="<?php echo esc_attr( $instance[ 'section_id'] ); ?>" placeholder="<?php esc_html_e( 'team', 'awesome-one-page' ); ?>">
                </div><!-- .aop-admin-input-wrap -->
    ...
    function widget( $args, $instance ) {
      ...
      $get_featured_pages = new WP_Query( array(
        ...
        'meta_query'      => array(
            array(
              ...
              'value' => 'page-templates/template-team.php'
            )
          )
      ) );
      ...
      <div <?php echo $section; ?> >
        <div class="widget aop-team-pages" style="<?php echo $background_style; ?>">
            ...
            <?php if ( $get_featured_pages->have_posts() ) : ?>
              <div class="entry-content">
                <?php while( $get_featured_pages->have_posts() ) : $get_featured_pages->the_post(); 
                  ...
                  $team_designation   = get_post_meta( $post->ID, 'awesome_one_page_team_designation', true );
                  $team_social_1      = get_post_meta( $post->ID, 'awesome_one_page_team_social_1', true );
                  $team_social_2      = get_post_meta( $post->ID, 'awesome_one_page_team_social_2', true );
                  $team_social_3      = get_post_meta( $post->ID, 'awesome_one_page_team_social_3', true );
                  if( has_post_thumbnail() ) {
                    $team_image_holder = '<figure><img src="'.esc_url( $image_path[0] ).'" alt="'.esc_attr( $image_alt ).'" title="'.esc_attr( $title_attribute ).'" /></figure>';
                  } ?>
                  <div class="team-column-3">
                    <?php if( has_post_thumbnail() || !empty( $team_icon ) ) : ?>
                      <div class="team-thumbnail">
                        <?php echo $team_image_holder; ?>
                      </div><!-- .thumbnail -->
                    <?php endif; ?>
                    ...
                    <?php if ( $team_social_1 || $team_social_2 || $team_social_3 ) : ?>
                      <ul class="team-social-icon">
                        <?php if ( $team_social_1 ) { ?>
                          <li><i class="fa fa-facebook"></i><?php echo esc_url( $team_social_1 ); ?></li>
                        <?php } if ( $team_social_2 ) { ?>
                          <li><i class="fa fa-twitter"></i><?php echo esc_url( $team_social_2 ); ?></li>
                        <?php } if ( $team_social_3 ) { ?> 
                          <li><i class="fa fa-google"></i><?php echo esc_url( $team_social_3 ); ?></li>
                        <?php } ?>
                      </ul><!-- .team-social-icon -->
                    <?php endif; ?>
