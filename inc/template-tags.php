...
if ( ! function_exists( 'awesome_one_page_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function awesome_one_page_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'awesome-one-page' ),
...
if ( ! function_exists( 'awesome_one_page_entry_footer' ) ) :
/**
...
function awesome_one_page_entry_footer() {
	...
	edit_post_link(
		sprintf(
			...
			esc_html__( 'Edit %s', 'awesome-one-page' ),
 ...
 * @return bool
 */
function awesome_one_page_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'awesome_one_page_categories' ) ) ) {
		...
		set_transient( 'awesome_one_page_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so awesome_one_page_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so awesome_one_page_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in awesome_one_page_categorized_blog.
 */
function awesome_one_page_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'awesome_one_page_categories' );
}
